...
class PyMongoProtocol(BufferedProtocol):
    ...
    def process_header(self) -> tuple[int, int]:
        ...
        length, _, response_to, op_code = _UNPACK_HEADER(self._buffer[self._start : self._start + 16])
        ...
        if op_code == 2012:
            ...
            if self._length >= 25:
                op_code, _, self._compressor_id = _UNPACK_COMPRESSION_HEADER(self._buffer[self._start + 16: self._start + 25])
            else:
