...
class PyMongoProtocol(BufferedProtocol):
        ...
        try:
            ...
            if self._done_messages:
                ...
                if self.transport and self.transport.is_closing():
                    raise OSError("Connection is closed because transport is closing")
            ...
            raise e
        raise OSError("connection closed no message")
    ...
    def buffer_updated(self, nbytes: int) -> None:
        ...
        if nbytes == 0:
            ...
            if self._overflow is not None:
                self._overflow_index = min(self._overflow_index + nbytes, self._body_size)
            else:
            ...
            if self._end_index + self._overflow_index - self._start_index >= self._body_size:
                ...
                if 16 > self._end_index - (self._start_index + self._body_size) > 0:
                    print(f"Have {self._end_index - (self._start_index + self._body_size)} bytes leftover from a message of size {(self._body_size + self._start_index) - self._start_index} and buffer of size {self._end_index} starting at {self._start_index}")
